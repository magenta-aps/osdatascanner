...
class TestRuleTranslation:
    ...
    def test_simple_translation_str(self, *, metadata, case_table):
        ...
        assert str(compiled_expr) == (
                'SELECT anon_1."Title" \n'
                'FROM (SELECT "Case"."Title" AS "Title", '
                'row_number() OVER '
                '(PARTITION BY "Case"."ID" ORDER BY "Case"."ID") '
                'AS _row_num \n'
                'FROM "Case" \n'
                'WHERE ("Case"."Title" LIKE \'%\' || :Title_1 || \'%\')) '
                'AS anon_1 \n'
                'WHERE anon_1._row_num = :row_num_1')
        assert compiled_expr.params["Title_1"] == "business"
        assert compiled_expr.params["row_num_1"] == 1
    ...
    def test_simple_translation_virtual_column(self, *, metadata, case_table):
        ...
        compiled_expr = sql_expr.compile()
        assert str(compiled_expr) == (
                'SELECT anon_1.datediff_1 \n'
                'FROM (SELECT datediff(day, "Case"."Created", now()) '
                'AS datediff_1, '
                'row_number() OVER '
                '(PARTITION BY "Case"."ID" ORDER BY "Case"."ID") '
                'AS _row_num \n'
                'FROM "Case" \n'
                'WHERE datediff(day, "Case"."Created", now()) >= '
                ':datediff_2) AS anon_1 \n'
                'WHERE anon_1._row_num = :row_num_1')
    ...
    def test_compound_translation_int(self, *, metadata, location_table):
        ...
        compiled_expr = sql_expr.compile()
        assert str(compiled_expr) == (
                'SELECT anon_1."PostNumber" \n'
                'FROM (SELECT "Location"."PostNumber" AS "PostNumber", '
                'row_number() OVER '
                '(PARTITION BY "Location"."ID" ORDER BY "Location"."ID") '
                'AS _row_num \n'
                'FROM "Location" \n'
    ...
    def test_complex_name_translation_str(self, *, metadata, case_table):
        ...
        assert str(compiled_expr) == (
                'SELECT anon_1."Name" \n'
                'FROM (SELECT "Location"."Name" AS "Name", '
                'row_number() OVER '
                '(PARTITION BY "Case"."ID" ORDER BY "Case"."ID") '
                'AS _row_num \n'
                ...
                'AND "Location"."Name" = :Name_1) AS anon_1 \n'
                'WHERE anon_1._row_num = :row_num_1')
    ...
    def test_complex_name_translation_str_neg(self, *, metadata, case_table):
        ...
        assert (compiled_expr1.params["Name_1"]
                ...
                == "Municipal Headquarters")
        assert (compiled_expr1.params["row_num_1"]
                == compiled_expr2.params["row_num_1"]
                == 1)
    ...
    def test_complex_name_cast_translation(self, *, metadata, partyinfo_table):
        ...
        compiled_expr = sql_expr.compile()
        assert str(compiled_expr) == (
                'SELECT anon_1."Name", anon_1."Name_1" \n'
                'FROM (SELECT "PartyType"."Name" AS "Name", '
                '"Person"."Name" AS "Name_1", '
                'row_number() OVER '
                '(PARTITION BY "PartyInfo"."ID" ORDER BY "PartyInfo"."ID") '
                'AS _row_num \n'
                ...
                'AND "Person"."Name" = :Name_3) AS anon_1 \n'
                'WHERE anon_1._row_num = :row_num_1')
    ...
    def test_complex_name_cast_translation_ref(self, *, metadata, case_table):
        ...
        compiled_expr = sql_expr.compile()
        assert str(compiled_expr) == (
                'SELECT anon_1."Municipality", anon_1."Municipality_1" \n'
                'FROM (SELECT "Location"."Municipality" AS "Municipality", '
        ...
        # The only bind parameter should be the dedup wrapper's row_num filter
        assert compiled_expr.params == {"row_num_1": 1}
    ...
    def test_complex_name_final_component_id(
        ...
        compiled_expr = sql_expr.compile()
        assert str(compiled_expr) == (
                'SELECT anon_1."PartyID" \n'
                'FROM (SELECT "PartyInfo"."PartyID" AS "PartyID", '
                'row_number() OVER '
                '(PARTITION BY "PartyInfo"."ID" ORDER BY "PartyInfo"."ID") '
                'AS _row_num \n'
                'FROM "PartyInfo" \n'
                'WHERE "PartyInfo"."PartyID" IS NOT NULL) AS anon_1 \n'
                'WHERE anon_1._row_num = :row_num_1')
        assert compiled_expr.params["row_num_1"] == 1
    ...
    def test_altkey_resolution_complex(
        ...
        assert str(compiled_expr) == (
                'SELECT anon_1."Name" \n'
                'FROM (SELECT "SecurityGroup"."Name" AS "Name", '
                'row_number() OVER '
                '(PARTITION BY "Case"."ID" ORDER BY "Case"."ID") '
                'AS _row_num \n'
                ...
                'AND "SecurityGroup"."Name" = :Name_1) AS anon_1 \n'
                'WHERE anon_1._row_num = :row_num_1')

        assert compiled_expr.params == {"Name_1": "Everybody", "row_num_1": 1}
