...
from os2datascanner.engine2.pipeline.processor import message_received, message_received_raw
...
def test_processor_aborts_before_conversion():
    ...
    msg = ConversionMessage(scan_spec=scan_spec, handle=handle, progress=progress)

    results = list(message_received(msg, SourceManager(), _check=False, should_abort=lambda: True))
    assert results == []
