...
class Realm(Exported, models.Model):
    realm_id = models.SlugField(
        allow_unicode=True,  # TODO: test that this works in Keycloak; if not: remove
        primary_key=True,
        verbose_name=_('realm id'),
    )
    organization = models.OneToOneField(
        'organizations.Organization',
        on_delete=models.PROTECT,  # TODO: To avoid data "leakage", ensure that the realm object is not deleted until the equivalent realm on Keycloak has been. Decide how to handle this
