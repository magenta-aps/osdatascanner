...
def handle_match_message(previous_report, new_report, body):
    ...
    logger.info(f"{new_matches.handle.presentation} has the matches: (rule, [matches])\n"
                f"{matches}")
        ...
        if previous_matches:
            ...
            if not new_matches.matched:
                ...
                if (len(new_matches.matches) == 1
                    ...
                    # XXX why is there no differemce between resolution status for
                    # XXX "no matches" and "matches stil present"
